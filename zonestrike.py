...
"""
ZoneStrike v1.7.2 — Interactive + Live Scan Output + Rich Reports (JSON/CSV/HTML)
...
ZoneStrike v{VERSION} — Interactive + Live Output + TCP Scan + Reporting
Use ONLY with explicit authorization.
...
def sprint(msg: str) -> None:
    # sync print (used during prompts)
    print(msg, flush=True)
...
def prompt_wordlist_filename() -> Optional[str]:
    base = script_dir()
    while True:
        s = input("Wordlist (arquivo no mesmo diretório) [Enter = pular]: ").strip()
        if not s:
            return None
        p = (base / s).resolve()
        if p.parent != base:
            print("  -> A wordlist deve estar NO MESMO diretório (sem subpastas / ../).")
            continue
...
def passive_discovery(target: str, timeout: float = 3.0) -> List[Tuple[str, str, str]]:
    ...
    r.timeout = timeout
    r.lifetime = timeout
    out: List[Tuple[str, str, str]] = [(target, "passive", "apex")]
...
def seeds_discovery(target: str) -> List[Tuple[str, str, str]]:
    return [(f"{s}.{target}", "seed", "embedded") for s in EMBEDDED_SEEDS]
...
async def run_discovery_live(
    target: str,
    wordlist_path: Optional[str],
    wordlist_limit: int,
    timeout: float = 3.0,
) -> Tuple[List[DiscoveryHit], Dict[str, str]]:
    items: List[Tuple[str, str, str]] = []
    items.extend(passive_discovery(target, timeout=timeout))
    items.extend(seeds_discovery(target))
    if wordlist_path:
        items.extend(wordlist_discovery(target, wordlist_path, limit=wordlist_limit))
    ...
    await aprint("\n[+] Discovery (resolvendo FQDNs em tempo real):")
    ...
    for fqdn in sorted(by_fqdn.keys()):
        ...
        hit = DiscoveryHit(fqdn=fqdn, source="multi", evidence=evidence, ips=ips)
        hits.append(hit)

        for ip in ips:
            ip_to_name.setdefault(ip, fqdn)

        await aprint(f"    - {fqdn} -> {', '.join(ips)}")
    ...
    if not hits:
        ...
        if ips:
            ...
            await aprint(f"    - {target} -> {', '.join(ips)} (fallback)")
...
async def scan_open_ports_live(
    ip: str,
    ports: List[int],
    concurrency: int,
    timeout: float,
    ...
    async def worker() -> None:
        ...
        while True:
            try:
                ...
                return
            try:
                ok = await try_connect(ip, p, timeout)
                if ok:
                    async with lock:
                        open_ports.append(p)
                    await aprint(f"      [OPEN] {host_label} {ip}:{p}/tcp")

                async with done_lock:
                    done += 1
                    # “heartbeat” a cada ~1s (pra você sentir que está vivo)
                    now = time.time()
                    if now - last_tick >= 1.0:
                        last_tick = now
                        await aprint(f"      [..] {host_label} progress {done}/{total}")
            finally:
                q.task_done()
...
async def enrich_ports_live(
    ...
    await aprint(f"    [+] Enrich: {host_label} ({ip}) em tempo real...")
    ...
    for p in open_ports:
        ...
        # live line per port
        svc = pf.service or "unknown"
        extra = ""
        if pf.http_status is not None:
            extra = f" HTTP:{pf.http_status}"
        await aprint(f"      [ENRICH] {host_label} {ip}:{p} -> {svc}{extra}")
...
async def main() -> None:
    ...
    target = prompt_nonempty("Target/domain (ex: lab.local)")
    top_n = prompt_int("Top N ports (ex: 100, 300, 1000)", 100, 1, 65535)
    report_name = prompt_nonempty("Nome do report (base)", "report")

    wordlist_path = prompt_wordlist_filename()
    wordlist_limit = 500
    if wordlist_path:
        wordlist_limit = prompt_int("Limite de linhas da wordlist", 500, 1, 200000)

    nmap_services = find_nmap_services_file()
    if not nmap_services:
        sprint("[!] nmap-services não encontrado. Instale: sudo apt install nmap")
        sys.exit(2)
    ...
    discovery, ip_to_name = await run_discovery_live(target, wordlist_path, wordlist_limit, timeout=3.0)
    all_ips = uniq([ip for d in discovery for ip in d.ips])

    await aprint(f"\n[+] Summary Discovery: FQDNs={len(discovery)} | Unique IPs={len(all_ips)}")
    if wordlist_path:
        await aprint(f"[+] Wordlist: {Path(wordlist_path).name} (limit={wordlist_limit})")
    else:
        await aprint("[+] Wordlist: (none)")
    ...
    await aprint(f"\n[+] Starting LIVE scan: hosts={total_hosts} | ports={len(ports)} (top{top_n})\n")

    sem = asyncio.Semaphore(host_concurrency)

    async def scan_one(ip: str, idx: int) -> HostReport:
        async with sem:
            host_label = ip_to_name.get(ip, target)
            await aprint(f"[SCAN] ({idx}/{total_hosts}) {host_label} -> {ip}")

            open_ports = await scan_open_ports_live(
                ip=ip,
                ports=ports,
            ...
            await aprint(f"    [DONE] {host_label} {ip} open_ports={len(open_ports)}")

            if not open_ports:
                return HostReport(ip=ip, name=host_label, open_ports=[], errors=[])

            enriched = await enrich_ports_live(
                ip=ip,
                host_label=host_label,
            ...
            return HostReport(ip=ip, name=host_label, open_ports=enriched, errors=[])

    tasks = [asyncio.create_task(scan_one(ip, i + 1)) for i, ip in enumerate(all_ips)]
    host_reports = await asyncio.gather(*tasks)
    ...
    await aprint(f"\n[+] Reports written: {json_path} and {csv_path}")
    ...
    await aprint(f"[+] HTML written: ./{html_dir}")
    await aprint(f"[+] Open report: {file_url}")
